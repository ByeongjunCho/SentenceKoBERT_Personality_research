...
from sentence_transformers_.sentence_transformers import SentencesDataset, LoggingHandler, util, InputExample
from sentence_transformers_.sentence_transformers.SentenceTransformer_split_classifier_bert import SentenceTransformer
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cuda', type=int, default=7, help='available gpu number(only multi-gpu)')
    parser.add_argument('--tied', type=bool, default=False, help='tied bert model during training')
    parser.add_argument('--model', type=str, default='BERT', help='give a model name')
    ...
    tail = '/3_updateAll/' + model_name.replace("/", "") + f'_epoch_{args.num_epochs}_batch_{args.batch_size}_tied_{str(args.tied)}_lr_{str(args.lr)}'
    model_save_path = './output' + tail


    # Apply mean pooling to get one fixed sized sentence vector
    model = SentenceTransformer("monologg/kobert")
    ...
    train_loss.load_state_dict(torch.load("/home/data5/byeongjuncho/SentenceEmbedding/output/1_tiedBERT_updateClassifier/tiedBERTModel_epoch_50_batch_32_tied_True_lr_5e-05/epoch_6_step_100_all_1180/loss_model.pt"))
