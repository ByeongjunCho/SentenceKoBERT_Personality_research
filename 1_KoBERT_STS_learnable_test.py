...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--num_epochs', type=int, default=80, help='give a epochs for training')
    ...
    parser.add_argument('--model', type=str, default='CustomSoftmaxLoss', help='give a model name')
    ...
    tail = '/NLI_architecture_test/' + model_name.replace("/", "") + f'_epoch_{args.num_epochs}_batch_{args.batch_size}_tied_{str(args.tied)}_lr_{str(args.lr)}'
    ...
    train_loss = losses.CustomSoftmaxLoss(model=model,
                                          sentence_embedding_dimension=model.get_sentence_embedding_dimension(),num_labels=1)
