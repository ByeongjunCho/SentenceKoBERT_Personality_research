...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--evaluation_step', type=int, default=50,
    ...
    parser.add_argument('--model', type=str, default='KoBERT', help='give a model name')
    ...
    tail = '/STS_Eudist_1dense_Classifier_AND_BERT_Training/' + model_name.replace("/", "") + f'_epoch_{args.num_epochs}_batch_{args.batch_size}_tied_{str(args.tied)}_lr_{str(args.lr)}_act_{args.act}'
    model_save_path = './output' + tail



    model = SentenceTransformer('./output/STS_histogram/tiedBERTModel_epoch_4_batch_16_tied_False_lr_1e-05/epoch_2_step_300_all_1020')
    ...
    train_loss.load_state_dict(torch.load(
        "output/STS_Eudist_1dense_tiedBERT/KoBERT_epoch_4_batch_16_tied_True_lr_2e-05_act_None/epoch_0_step_250_all_250/loss_model.pt"))
