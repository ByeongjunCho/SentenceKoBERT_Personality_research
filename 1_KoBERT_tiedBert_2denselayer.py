...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='tiedBERTModel', help='give a model name')
    ...
    tail = '/1_CustomSTS_tied_noAct_2denselayer/' + model_name.replace("/", "") + f'_epoch_{args.num_epochs}_batch_{args.batch_size}_tied_{str(args.tied)}_lr_{str(args.lr)}'
    ...
    train_loss = losses.CustomSoftmaxLoss(model=model, sentence_embedding_dimension=model.get_sentence_embedding_dimension(), num_labels=1, concatenation_sent_difference=False)
    ...
    if args.tied:
        for name, param in train_loss.named_parameters():
            if name in ['classifier1.weight', 'classifier1.bias', 'classifier2.weight', 'classifier2.bias']:
